...
def test_git_identity_ok(monkeypatch):
    ...
    r = git_identity.GitIdentityProvider().observe(_ctx("FLE-IDENTITY-101"))
...
def test_git_identity_enforce_sets_persona(monkeypatch):
    ...
    r = git_identity.GitIdentityProvider().enforce(_ctx("FLE-IDENTITY-101"))
...
def test_environment_clean(monkeypatch):
    ...
    r = environment.EnvironmentSecretProvider().observe(_ctx("FLE-SECRET-050"))
    assert r.state is State.OK
...
def test_forbidden_files_absent(monkeypatch):
    ...
    ctx = _ctx("FLE-SECRET-060", params={"forbidden_paths": ["~/.netrc"]})
...
def test_forbidden_files_present(monkeypatch):
    ...
    ctx = _ctx("FLE-SECRET-060", params={"forbidden_paths": ["~/.aws/credentials"]})
    r = filesystem.ForbiddenFilesProvider().observe(ctx)
    assert r.state is State.VIOLATION
...
def test_vpn_present(monkeypatch):
    ...
    ctx = _ctx("FLE-EGRESS-001", params={"interface": "wg0"})
...
def test_disk_encrypted(monkeypatch):
    ...
    r = disk.DiskEncryptionProvider().observe(_ctx("FLE-DISK-001"))
    assert r.state is State.OK
