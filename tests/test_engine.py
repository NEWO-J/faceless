...
def test_conformant_when_all_ok(monkeypatch):
    ...
    report = evaluate(_config("FLE-DISK-001"))
...
def test_git_drift_makes_non_conformant(monkeypatch):
    ...
    report = evaluate(_config("FLE-IDENTITY-101"))
    assert not report.conformant
    assert report.results[0].state is State.DRIFT


def test_fast_skips_expensive_without_cache():
    report = evaluate(_config("FLE-DISK-001"), fast=True, cache=None)
    assert report.results[0].state is State.NOT_ENFORCED


def test_fast_uses_cache_when_present():
    cached = {"FLE-DISK-001": CheckResult(
        control_id="FLE-DISK-001", state=State.OK, severity=Severity.HIGH, summary="cached")}
    report = evaluate(_config("FLE-DISK-001"), fast=True, cache=cached)
...
def test_drift_from_lock(monkeypatch):
    ...
    lock = {"FLE-DISK-001": {"encrypted": "false"}}
    report = evaluate(_config("FLE-DISK-001"), lock=lock)
    assert report.results[0].state is State.DRIFT
...
def test_converge_remediates_git(monkeypatch):
    ...
    config = _config("FLE-IDENTITY-101")
