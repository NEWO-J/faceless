...
def test_egress_route_ok(monkeypatch):
    ...
    r = network.EgressRouteProvider().observe(_ctx("FLE-EGRESS-002", {"interface": "wg0"}))
...
def test_dns_leak(monkeypatch):
    ...
    ctx = _ctx("FLE-NET-001", {"allowed_resolvers": ["10.0.0.1"]})
...
def test_firewall_default_deny(monkeypatch):
    ...
    assert network.FirewallPolicyProvider().observe(_ctx("FLE-NET-003")).state is State.OK
    monkeypatch.setattr(network, "_firewall_policies", lambda: {"OUTPUT": "ACCEPT"})
    assert network.FirewallPolicyProvider().observe(_ctx("FLE-NET-003")).state is State.VIOLATION
    monkeypatch.setattr(network, "_firewall_policies", lambda: None)
    assert network.FirewallPolicyProvider().observe(_ctx("FLE-NET-003")).state is State.ERROR


def test_listening_ports(monkeypatch):
    monkeypatch.setattr(network, "_listening_ports", lambda: [22, 8080])
    ctx = _ctx("FLE-NET-004", {"allowed_ports": [22]})
...
def test_non_linux_is_not_applicable(monkeypatch):
    ...
    r = network.FirewallPolicyProvider().observe(_ctx("FLE-NET-003"))
...
def test_encrypted_dns(monkeypatch):
    ...
    assert network.EncryptedDnsProvider().observe(_ctx("FLE-NET-005")).state is State.OK
    monkeypatch.setattr(network, "_resolved_protocol", lambda f: {"DNSOverTLS": False}.get(f))
    assert network.EncryptedDnsProvider().observe(_ctx("FLE-NET-005")).state is State.VIOLATION


def test_resolver_is_gateway(monkeypatch):
    monkeypatch.setattr(network, "_default_gateway", lambda: "192.168.1.1")
    monkeypatch.setattr(network, "_nameservers", lambda: ["192.168.1.1"])
    assert network.ResolverIsGatewayProvider().observe(_ctx("FLE-NET-006")).state is State.VIOLATION
    monkeypatch.setattr(network, "_nameservers", lambda: ["10.64.0.1"])
    assert network.ResolverIsGatewayProvider().observe(_ctx("FLE-NET-006")).state is State.OK


def test_llmnr(monkeypatch):
    monkeypatch.setattr(network, "_resolved_protocol", lambda f: True if f == "LLMNR" else None)
    assert network.LlmnrProvider().observe(_ctx("FLE-NET-007")).state is State.VIOLATION
    monkeypatch.setattr(network, "_resolved_protocol", lambda f: False if f == "LLMNR" else None)
    assert network.LlmnrProvider().observe(_ctx("FLE-NET-007")).state is State.OK
...
def test_mdns_broadcasting(monkeypatch):
    ...
    assert network.MdnsProvider().observe(_ctx("FLE-NET-008")).state is State.VIOLATION
    monkeypatch.setattr(network, "_service_active", lambda n: False)
    assert network.MdnsProvider().observe(_ctx("FLE-NET-008")).state is State.OK


def test_ipv6_privacy(monkeypatch):
    monkeypatch.setattr(network, "_ipv6_disabled", lambda: False)
    monkeypatch.setattr(network, "_use_tempaddr", lambda: 2)
    assert network.Ipv6PrivacyProvider().observe(_ctx("FLE-NET-009")).state is State.OK
    monkeypatch.setattr(network, "_use_tempaddr", lambda: 0)
    assert network.Ipv6PrivacyProvider().observe(_ctx("FLE-NET-009")).state is State.VIOLATION


def test_ipv6_eui64_leak(monkeypatch):
    monkeypatch.setattr(network, "_global_ipv6_addrs", lambda: ["2001:db8::211:22ff:fe33:4455"])
    assert network.Ipv6Eui64Provider().observe(_ctx("FLE-NET-010")).state is State.VIOLATION
    monkeypatch.setattr(network, "_global_ipv6_addrs", lambda: ["2001:db8::dead:beef:cafe:1234"])
    assert network.Ipv6Eui64Provider().observe(_ctx("FLE-NET-010")).state is State.OK
...
def test_mac_randomization(monkeypatch):
    ...
    assert network.MacRandomizationProvider().observe(_ctx("FLE-NET-011")).state is State.OK
    monkeypatch.setattr(network, "_nm_print_config", lambda: "wifi.powersave=2\n")
    assert network.MacRandomizationProvider().observe(_ctx("FLE-NET-011")).state is State.VIOLATION


def test_connectivity_check(monkeypatch):
    monkeypatch.setattr(network, "_nm_print_config", lambda: "[connectivity]\nenabled=false\n")
    assert network.ConnectivityCheckProvider().observe(_ctx("FLE-NET-012")).state is State.OK
    monkeypatch.setattr(network, "_nm_print_config", lambda: "[connectivity]\nuri=http://x/check\n")
    assert network.ConnectivityCheckProvider().observe(_ctx("FLE-NET-012")).state is State.VIOLATION


def test_tcp_timestamps(monkeypatch):
    monkeypatch.setattr(network, "_sysctl_int", lambda k: 0)
    assert network.TcpTimestampsProvider().observe(_ctx("FLE-NET-013")).state is State.OK
...
def test_public_ip_leak(monkeypatch):
    ...
    ctx = _ctx("FLE-NET-014", {"forbidden_ips": ["203.0.113.9"]})
    assert network.PublicIpProvider().observe(ctx).state is State.VIOLATION
    ctx_ok = _ctx("FLE-NET-014", {"forbidden_ips": ["203.0.113.9"]})
